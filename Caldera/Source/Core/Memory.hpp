...
template <class Ty> using Ref = std::shared_ptr<Ty>;
...
template <class Ty> class Allocator
{
  ...
  Allocator(Memory::MemoryTag tag)
      : tag(tag)
  {
  }

  Memory::MemoryTag getTag() const noexcept
  {
    return tag;
  }
  ...
  Ty* allocate(std::size_t n) noexcept
  {
    return (Ty*)Memory::allocateMemory(n * sizeof(Ty), tag);
  }

  void deallocate(Ty* p, std::size_t n) noexcept
  {
    Memory::freeMemory(p, tag);
  }
};
