...
class AssetManager
{
  ...
  {
    if (meshMap.find(handle) != meshMap.end())
      return meshMap.find(handle)->second;
    return nullptr;
  }
  ...
  MeshHandle loadMesh(std::filesystem::path path, Ref<Renderer>& renderer)
  {
    ...
    asset->vertexBuffer =
        renderer->createVertexBuffer(meshData.vertices.data(), meshData.vertices.size());
    asset->indexBuffer =
        renderer->createIndexBuffer(meshData.indices.data(), meshData.indices.size());
