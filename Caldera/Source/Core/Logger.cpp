...
FILE* StreamFor(LogLevel level)
{
  return (level == LogLevel::Fatal || level == LogLevel::Error) ? stderr : stdout;
}
...
void logOutput(LogLevel level, std::source_location loc, StringView message)
{
  ...
  String formatted =
      std::format("{}[{}] {}:{} ({}) - {}{}\n", LevelColor(level), LevelLabel(level),
                  loc.file_name(), loc.line(), loc.function_name(), message, ColorReset);
