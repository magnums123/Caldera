...
template <typename Ty> class Vector
{
  ...
  Vector()
      ...
      , mData((Ty*)allocateMemory(defaultCapacity * sizeof(Ty), MemoryTag::DARRAY))
  {
  }
  ...
  Vector(size_t size)
      : mSize(size)
      , mCapacity(size)
      , mData((Ty*)allocateMemory(size * sizeof(Ty), MemoryTag::DARRAY))
  {
  ...
  {
    size_t i = 0;
    for (const Ty& item : data)
      constructAt(mData + i++, item);
  }
  ...
  {
    for (size_t i = 0; i < mSize; ++i)
      constructAt(mData + i, other.mData[i]);
  }
  ...
  {
    if (mSize == mCapacity)
      reallocateTo(mCapacity == 0 ? defaultCapacity : mCapacity * 2);
  ...
  private:
  static void constructAt(Ty* ptr)
  {
    ::new (static_cast<void*>(ptr)) Ty();
  }
  static void constructAt(Ty* ptr, const Ty& value)
  {
    ::new (static_cast<void*>(ptr)) Ty(value);
  }
  static void constructAt(Ty* ptr, Ty&& value)
  {
    ::new (static_cast<void*>(ptr)) Ty(std::move(value));
  }
  static void destroyAt(Ty* ptr)
  {
    ptr->~Ty();
  }
  ...
  void releaseBuffer()
  {
    destroyAll();
    if (mData)
      freeMemory(mData, MemoryTag::DARRAY);
    mSize = 0;
    mCapacity = 0;
    mData = nullptr;
  }
  ...
  void reallocateTo(size_t newCapacity)
  {
    ...
    for (size_t i = 0; i < mSize; ++i)
      constructAt(newData + i, std::move(mData[i]));
    destroyAll();
    if (mData)
      freeMemory(mData, MemoryTag::DARRAY);
    mData = newData;
