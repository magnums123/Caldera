...
VulkanSwapchain::VulkanSwapchain(const VulkanContext& context, uint32_t width, uint32_t height)
{
  ...
  width = std::clamp(width, context.device->swapchainInfo.surfaceCapabilities.minImageExtent.width,
                     context.device->swapchainInfo.surfaceCapabilities.maxImageExtent.width);
  height =
      std::clamp(height, context.device->swapchainInfo.surfaceCapabilities.minImageExtent.height,
  ...
  vk::SwapchainCreateInfoKHR swapchainCreateInfo{
      ...
      .minImageCount = context.device->swapchainInfo.surfaceCapabilities.minImageCount,
  ...
  swapchain =
      context.device->logicalDevice.createSwapchainKHR(swapchainCreateInfo, context.allocator);

  for (const auto& image : context.device->logicalDevice.getSwapchainImagesKHR(swapchain))
  ...
  for (auto format : possibleDepthFormats)
  {
    vk::FormatProperties formatProperties =
        context.device->physicalDevice.getFormatProperties(format);
    if (formatProperties.optimalTilingFeatures & vk::FormatFeatureFlagBits::eDepthStencilAttachment)
      depthImageFormat = format;
  }

  ASSERT_MSG(depthImageFormat != vk::Format::eUndefined,
             "Failed to find suitable Format for Depth Image");
  ...
  vma::AllocationCreateInfo allocationCreateInfo{
      .flags = vma::AllocationCreateFlagBits::eDedicatedMemory, .usage = vma::MemoryUsage::eAuto};
  auto imageAndAllocation =
      context.vmaAllocator.createImage(depthImageCreateInfo, allocationCreateInfo);
  ...
  depthImageView =
      context.device->logicalDevice.createImageView(depthImageViewCreateInfo, context.allocator);
}

VulkanSwapchain::~VulkanSwapchain() {}

vk::SurfaceFormatKHR
VulkanSwapchain::chooseBestSurfaceFormat(const Vector<vk::SurfaceFormatKHR>& formats)
{
...
vk::PresentModeKHR
VulkanSwapchain::chooseBestPresentMode(const Vector<vk::PresentModeKHR>& presentModes)
{
  // TODO: Choose the actual Best Format
